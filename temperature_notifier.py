...
class TempMonitor():
    def __init__(self, config_path: str, device_name: str, temp_thresholds: dict=None) -> None:
    ...
    def _read_config(self):
        ...
        if not config.has_section('Telegram') or "bot_token" not in config['Telegram'] or "chat_id" not in config['Telegram']:
            self.logger.critical(
                "Telegram bot token or chat ID not found in config file. Exiting...")
            
            self.logger.info(f"Read from config.ini: `{config.read(self.config_path)}`")
            raise ValueError("Telegram bot token or chat ID not found in config.ini file. This could be due to the format of the config file.")
    ...
    def get_temp(self) -> float:
        ...
        try: 
            ...
            temp_float_val: float = float(temp.replace("temp=","").replace("'C\n",""))
    ...
    def monitor_temp(self, temp: float) -> None:
        ...
        for threshold, message in self.temp_thresholds.items():
            if temp > threshold:
                ...
                self.send_notification(f"*WARNING - TEMPERATURE EXCEEDS {threshold}c.*\n{message}: {temp}c!")
                break
...
if __name__ == '__main__':
    ...
    temp_monitor = TempMonitor(config_path=args.config_file, device_name=args.device_name)
    temp_monitor.run()
